...
public class LinkedIntList {
    ...
    public static LinkedIntList concatenated(LinkedIntList A, LinkedIntList B) {
        ...
        helperConcat(newList, A);
        helperConcat(newList, B);
        return newList;
    }

    /**
     * Add the list B to the end of list of A by copying each element in B if
     * there exists such elements (Modify A but do not modify B)
     */
    private static void helperConcat(LinkedIntList newList, LinkedIntList A) {
        if (newList == null) {
            return;
        }
        ListNode backN = null;
        ...
        if (A != null && A.front != null) {
            if (backN == null) {
                ...
                backN.next = new ListNode(A.front.data);
                backN = backN.next;
            }
            ListNode backA = A.front;
            while (backA.next != null) {
                backN.next = new ListNode(backA.next.data);
                backN = backN.next;
                backA = backA.next;
            }
        }
